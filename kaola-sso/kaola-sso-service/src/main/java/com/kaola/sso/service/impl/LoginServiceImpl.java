...
public class LoginServiceImpl implements LoginService {
	...
	@Override
	public KLResult userLogin(String username, String password) {
		// 1、判断用户和密码是否正确
		//执行查询
		List<TbUser> list = userDao.queryUser(username,null);
		if (list == null || list.size() == 0) {
			//返回登录失败
			return KLResult.build(400, "用户名或密码错误");
		}
		//取用户信息
